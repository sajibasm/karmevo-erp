...
| ~~Staff app on tenant subdomain vs single `app.erp.com`~~ | — | **Resolved 2026-09-23 (ADR-0012):** per-tenant subdomains for every surface, staff app included. Slug allocation/change policy and custom-domain verification remain open, scheduled with P1.1b-2 |
...
| **P1.1b-1 Permission engine** (detailed plan written) | AUTH-01..12, IAM-01/04, APR-01 (policy side). New core `authz` module: permission definitions, groups + templates, group policies, user overrides, authorization revisions, permission-change audit. Ordered AUTH-04 evaluation with deny precedence and AUTH-05 no-merge, typed condition tree, record scope, approval limits, separation of duties, delegation limits. Group/override editor API and effective-permissions inspector. **API only** (owner decision 2026-09-23); `erp-web` shell deferred | All 13 AUTH-12 cases via API; deny precedence; disjoint-branch-limit case; delegation limits | 1a |
| P1.1b-2 Tenant domains | ADR-0012: `registry."TenantDomains"`, host → tenant middleware with trusted-proxy validation (OPS-02), reserved slugs, per-tenant Keycloak client provisioning, domain-ownership verification (DB-03). Resolves the ADR-0012 open sub-decisions | Host resolves to tenant; unverified/forged host rejected; host never overrides membership; reserved slug refused | 1a, 1b-1 |
...
2. P1.1b-1 → P1.1d, each getting its own detailed plan written when the previous one is merged, so plans reflect real code. P1.1b was split into P1.1b-1 (permission engine) and P1.1b-2 (tenant domains) on 2026-09-23: they share no code, and only the second depends on ADR-0012.
